export const data_images = [
  {
    image: "./images/nguyen.png",
    id: 1,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/new.jpg",
    id: 2,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents: "Em đẹp lắm Ichika 💗",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/nguyen.png",
    id: 3,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "CHÚ CHÓ MAY MẮN KHI CÓ MỘT GIA ĐÌNH YÊU THƯƠNG Một người mẹ tên Joely Eaton, 29 tuổi tại Anh Quốc có nuôi một chú chó tên Buddy, nhưng không may nó lại mắc một loại u.ng t.hư xương ác tính. Vì thế Joely đã phải chi trả một hoá đơn lên đến hơn 8000 bảng Anh (hơn 200 triệu đồng) cho việc c-ắ-t b-ỏ ch-ân của chú chó này.",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/new.jpg",
    id: 4,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Hôm qua bắt Argen ăn được 500k nên sáng sớm đưa vợ đi uống cafe 🐧",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/nguyen.png",
    id: 5,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/new.jpg",
    id: 6,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/nguyen.png",
    id: 7,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/new.jpg",
    id: 8,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  {
    image: "./images/nguyen.png",
    id: 9,
    user: "Nguyen Hoang",
    id_user: "@nguyenhoang",
    avatar: "./images/nguyen.png",
    contents:
      "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
    data: [
      {
        name: "Tuan Le",
        avatar: "./images/nguyen.png",
        comment:
          "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
      },
      {
        name: "Nguyen Hoang",
        avatar: "./images/new.jpg",
        comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
      {
        name: "Huy Huynh",
        avatar: "./images/logo/logo1.png",
        comment:
          "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
      },
    ],
  },
  //   {
  //     image: "./images/new.jpg",
  //     id: 10,
  //     user: "Nguyen Hoang",
  //     id_user: "@nguyenhoang",
  //     avatar: "./images/nguyen.png",
  //     contents: "Em đẹp lắm Ichika 💗",
  //     data: [
  //       {
  //         name: "Tuan Le",
  //         avatar: "./images/nguyen.png",
  //         comment:
  //           "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
  //       },
  //       {
  //         name: "Nguyen Hoang",
  //         avatar: "./images/new.jpg",
  //         comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //     ],
  //   },
  //   {
  //     image: "./images/new.jpg",
  //     id: 11,
  //     user: "Nguyen Hoang",
  //     id_user: "@nguyenhoang",
  //     avatar: "./images/nguyen.png",
  //     contents: "Em đẹp lắm Ichika 💗",
  //     data: [
  //       {
  //         name: "Tuan Le",
  //         avatar: "./images/nguyen.png",
  //         comment:
  //           "Các bạn thông cảm, anh qua Qatar coi đá banh dới mét si dới rô nán đồ gòi",
  //       },
  //       {
  //         name: "Nguyen Hoang",
  //         avatar: "./images/new.jpg",
  //         comment: "Aisshh chít tịt sao Rosé (BlackPink) lại xinh như vậy chứ 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //       {
  //         name: "Huy Huynh",
  //         avatar: "./images/logo/logo1.png",
  //         comment:
  //           "Hình ảnh cảm lạnh cùng những tiếng gào thét bất lực khi dạy bài cho em 🥹",
  //       },
  //     ],
  //   },
];
export const data_user_other = [
  {
    name: "Nhat Huy",
    id: "@nhathuy",
    avatar: "./images/nguyen.png",
    about: "dev lỏd",
  },
  {
    name: "Tuan le",
    id: "@tuanle",
    avatar: "./images/new.jpg",
    about: "dev lỏd",
  },
  {
    name: "Toi Di Code Dao",
    id: "@toidicodedao",
    avatar: "./images/nguyen.png",
    about: "...",
  },
  {
    name: "MTP",
    id: "@mtp",
    avatar: "./images/new.jpg",
    about: "singer",
  },
  {
    name: "Sugar Daddy",
    id: "@sugardaddy",
    avatar: "./images/nguyen.png",
    about: "trap boy",
  },
];
export const data_user = {
  name: "Nguyen Hoang",
  id: "@nguyenhoang",
  avatar: "./images/new.jpg",
  location: "Location",
  about: "About",
  gender: "male",
  dayOfBirth: "2005-11-11",
};
export const data_blog = [
  {
    url: "https://shopee.vn",
    image: "./images/shopee.png",
    date: "20-10-2022",
    content: "shopeeasdh asjdh nkja sdkjha skjdkjash dkajshdakjshd kajhsd",
  },
  {
    url: "https://shopee.vn",
    image: "./images/shopee.png",
    date: "20-10-2022",
    content: "shopee",
  },
  {
    url: "https://shopee.vn",
    image: "./images/shopee.png",
    date: "20-10-2022",
    content: "shopee",
  },
];
